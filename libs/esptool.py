...
class ESPLoader(object):
    ...
    def flash_begin(self, size, offset):
        ...
        if size != 0 and not self.IS_STUB:
            _log("Took %.2fs to erase flash block" % (time.time() - t))
        self._port.timeout = DEFAULT_TIMEOUT
        return num_blocks
    ...
    def flash_defl_begin(self, size, compsize, offset):
        ...
        _log("Compressed %d bytes to %d..." % (size, compsize))
    ...
    def change_baud(self, baud):
        _log("Changing baud rate to %d" % baud)
    ...
    def warn_if_unusual_segment(self, offset, size, is_irom_segment):
        if not is_irom_segment:
            if offset > 0x40200000 or offset < 0x3ffe0000 or size > 65536:
                _log('WARNING: Suspicious segment 0x%x, length %d' % (offset, size))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if segments != ESPBOOTLOADER.IMAGE_V2_SEGMENT:
                ...
                _log('Warning: V2 header has unexpected "segment" count %d (usually 4)' % segments)
            ...
            if first_flash_mode != self.flash_mode:
                _log('WARNING: Flash mode value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_mode, self.flash_mode))
            if first_flash_size_freq != self.flash_size_freq:
                _log('WARNING: Flash size/freq value in first header (0x%02x) disagrees with second (0x%02x). Using second value.'
                      % (first_flash_size_freq, self.flash_size_freq))
            if first_entrypoint != self.entrypoint:
                _log('WARNING: Entrypoint address in first header (0x%08x) disagrees with second header (0x%08x). Using second value.'
                      % (first_entrypoint, self.entrypoint))
    ...
    def __init__(self, load_file=None):
        ...
        if load_file is not None:
            ...
            if additional_header != self.EXTENDED_HEADER:
                _log("WARNING: ESP32 image header contains unknown flags. Possibly this image is from a different version of esptool.py")
...
class ELFFile(object):
    ...
    def _read_sections(self, f, section_header_offs, section_header_count, shstrndx):
        ...
        if sec_type != ELFFile.SEC_TYPE_STRTAB:
            _log('WARNING: ELF file has incorrect STRTAB section type 0x%02x' % sec_type)
        f.seek(sec_offs)
...
def load_ram(esp, args):
    ...
    for (offset, size, data) in image.segments:
        _log('Downloading %d bytes at %08x...' % (size, offset), end=' ')
        sys.stdout.flush()
    ...
    _log('All segments done, executing at %08x' % image.entrypoint)
    esp.mem_finish(image.entrypoint)


def read_mem(esp, args):
    _log('0x%08x = 0x%08x' % (args.address, esp.read_reg(args.address)))


def write_mem(esp, args):
    esp.write_reg(args.address, args.value, args.mask, 0)
    _log('Wrote %08x, mask %08x to %08x' % (args.value, args.mask, args.address))
...
def detect_flash_size(esp, args):
    if args.flash_size == 'detect':
        ...
        if args.flash_size is None:
            _log('Warning: Could not auto-detect Flash size (FlashID=0x%x, SizeID=0x%x), defaulting to 4MB' % (flash_id, size_id))
            args.flash_size = '4MB'
        else:
            _log('Auto-detected Flash size:', args.flash_size)
...
def _update_image_flash_params(esp, address, args, image):
    ...
    if flash_params != image[2:4]:
        _log('Flash params set to 0x%04x' % struct.unpack(">H", flash_params))
...
def write_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        while len(image) > 0:
            _log('Writing at 0x%08x... (%d %%)' % (address + seq * esp.FLASH_WRITE_SIZE, 100 * (seq + 1) // blocks))
            sys.stdout.flush()
        ...
        if args.compress:
            ...
            _log('Wrote %d bytes (%d compressed) at 0x%08x in %.1f seconds%s...' % (uncsize, written, address, t, speed_msg))
        else:
            if t > 0.0:
                speed_msg = " (%.1f kbit/s)" % (written / t * 8 / 1000)
            _log('Wrote %d bytes at 0x%08x in %.1f seconds%s...' % (written, address, t, speed_msg))
        try:
            res = esp.flash_md5sum(address, uncsize)
            if res != calcmd5:
                _log('File  md5: %s' % calcmd5)
                _log('Flash md5: %s' % res)
                _log('MD5 of 0xFF is %s' % (hashlib.md5(b'\xFF' * uncsize).hexdigest()))
                raise FatalError("MD5 of file does not match data in flash!")
            else:
                _log('Hash of data verified.')
    ...
    if args.verify:
        _log('Verifying just-written flash...')
        _log('(This option is deprecated, flash contents are now always read back after flashing.)')
        verify_flash(esp, args)


def image_info(args):
    image = LoadFirmwareImage(args.chip, args.filename)
    _log('Image version: %d' % image.version)
    _log('Entry point: %08x' % image.entrypoint if image.entrypoint != 0 else 'Entry point not set')
    _log('%d segments' % len(image.segments))
    print
    idx = 0
    for seg in image.segments:
        idx += 1
        _log('Segment %d: %r' % (idx, seg))
    calc_checksum = image.calculate_checksum()
    _log('Checksum: %02x (%s)' % (image.checksum,
...
def elf2image(args):
    ...
    if args.chip == 'auto':  # Default to ESP8266 for backwards compatibility
        _log("Creating image for ESP8266...")
        args.chip == 'esp8266'
...
def read_mac(esp, args):
    ...
    def print_mac(label, mac):
        _log('%s: %s' % (label, ':'.join(map(lambda x: '%02x' % x, mac))))
    print_mac("MAC", mac)


def chip_id(esp, args):
    chipid = esp.chip_id()
    _log('Chip ID: 0x%08x' % chipid)


def erase_flash(esp, args):
    _log('Erasing flash (this may take a while)...')
    t = time.time()
    esp.erase_flash()
    _log('Chip erase completed successfully in %.1fs' % (time.time() - t))


def erase_region(esp, args):
    _log('Erasing region (may be slow depending on size)...')
    t = time.time()
    esp.erase_region(args.address, args.size)
    _log('Erase completed successfully in %.1f seconds.' % (time.time() - t))
...
def flash_id(esp, args):
    flash_id = esp.flash_id()
    _log('Manufacturer: %02x' % (flash_id & 0xff))
    flid_lowbyte = (flash_id >> 16) & 0xFF
    _log('Device: %02x%02x' % ((flash_id >> 8) & 0xff, flid_lowbyte))
    _log('Detected flash size: %s' % (DETECTED_FLASH_SIZES.get(flid_lowbyte, "Unknown")))
...
def read_flash(esp, args):
    ...
    t = time.time() - t
    _log('Read %d bytes at 0x%x in %.1f seconds (%.1f kbit/s)...'
...
def verify_flash(esp, args):
    ...
    for address, argfile in args.addr_filename:
        ...
        _log('Verifying 0x%x (%d) bytes @ 0x%08x in flash against %s...' % (image_size, image_size, address, argfile.name))
        # Try digest first, only read if there are differences.
        digest = esp.flash_md5sum(address, image_size)
        expected_digest = hashlib.md5(image).hexdigest()
        if digest == expected_digest:
            _log('-- verify OK (digest matched)')
            continue
        else:
            differences = True
            if getattr(args, 'diff', 'no') != 'yes':
                _log('-- verify FAILED (digest mismatch)')
                continue

        flash = esp.read_flash(address, image_size)
        assert flash != image
        diff = [i for i in range(image_size) if flash[i] != image[i]]
        _log('-- verify FAILED: %d differences, first @ 0x%08x' % (len(diff), address + diff[0]))
        for d in diff:
            flash_byte = flash[d]
            image_byte = image[d]
            if PYTHON2:
            ...
            _log('   %08x %02x %02x' % (address + d, flash_byte, image_byte))
    if differences:
        raise FatalError("Verify failed.")


def read_flash_status(esp, args):
    _log('Status value: 0x%04x' % esp.read_status(args.bytes))


def write_flash_status(esp, args):
    fmt = "0x%%0%dx" % (args.bytes * 2)
    args.value = args.value & ((1 << (args.bytes * 8)) - 1)
    _log(('Initial flash status: ' + fmt) % esp.read_status(args.bytes))
    _log(('Setting flash status: ' + fmt) % args.value)
    esp.write_status(args.value, args.bytes, args.non_volatile)
    _log(('After flash status:   ' + fmt) % esp.read_status(args.bytes))
...
def main():
    ...
    _log('esptool.py v%s' % __version__)
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        _log("Chip is %s" % (esp.get_chip_description()))
        ...
        if args.baud > initial_baud:
            try:
                ...
                _log("WARNING: ROM doesn't support changing baud rate. Keeping initial baud rate %d" % initial_baud)

        # override common SPI flash parameter stuff if configured to do so
        if hasattr(args, "spi_connection") and args.spi_connection is not None:
            if esp.CHIP_NAME != "ESP32":
                raise FatalError("Chip %s does not support --spi-connection option." % esp.CHIP_NAME)
            _log("Configuring SPI flash mode...")
            esp.flash_spi_attach(args.spi_connection)
        elif args.no_stub:
            _log("Enabling default SPI flash mode...")
            # ROM loader doesn't enable flash unless we explicitly do it
            esp.flash_spi_attach(0)

        if hasattr(args, "flash_size"):
            _log("Configuring flash size...")
...
def expand_file_arguments():
    ...
    for arg in sys.argv:
        if arg.startswith("@"):
            ...
            new_args.append(arg)
    if expanded:
        _log("esptool.py %s" % (" ".join(new_args[1:])))
        sys.argv = new_args
...
class FlashSizeAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            _log("WARNING: Flash size arguments in megabits like '%s' are deprecated." % (values[0]))
            _log("Please use the equivalent size '%s'." % (value))
            _log("Megabit arguments may be removed in a future release.")
        except KeyError:
            value = values[0]
